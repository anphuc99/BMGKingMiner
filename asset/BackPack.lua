function self:onOpen(p)
    ...
    local function setItem(v,cellNum)
        ...
        self.BackPack.CellBP["cell"..cellNum].Item:setImage("gameres|"..v.icon)
    ...
    local function reloadBackPack()
        ...
        hasItem = {}
        lisItem = {}
        PackageHandlers.sendClientHandler("getBackPackPlayer", nil,function(data)
            print(Lib.pv(data))
    ...
    local function cellDbClick(i)
        ...
        if hasItem[i] then       
            lisItem[i].cellNum = i     
            PackageHandlers.sendClientHandler("baloToHand", lisItem[i], function (rs)
                print(rs)
                if rs then
                    self.BackPack.CellBP["cell" .. i].Item:setVisible(false)
                    self.BackPack.CellBP["cell" .. i].Image1:setVisible(false)
        ...
        -- chuyển từ tay lên balo 
            ...
            if i <= balo then
                PackageHandlers.sendClientHandler("handToBalo", {cellNum = i}, function (v)
                    print(Lib.pv(v))
                    setItem(v,i)
                end)
            end
    ...
    local function cellClick(i)
       ...
       if i <= balo then
            if curClick == nil or not hasItem[curClick] then
                ...
                PackageHandlers.sendClientHandler("moveItem", {newCell = i, oldCell = curClick}, function (rep)
                    if rep.rs then
                        if rep.oldCell ~= nil then
                            setItem(rep.oldCell,curClick)
                        else
                            self.BackPack.CellBP["cell" .. curClick].Item:setVisible(false)
                            self.BackPack.CellBP["cell" .. curClick].Image1:setVisible(false)
                            hasItem[i] = nil
                            lisItem[i] = nil
                        end
