...
Trigger.RegisterHandler(this, "ENTITY_ENTER", function(context) 
    ...
    if(not newPlayer) then
        ...
        local playerItem = {
            {idPlayer = context.obj1.platformUserId, idItem = cupLv1,cellNum = 1, num = 1, position = positionItem.hand, lv = 1},
            {idPlayer = context.obj1.platformUserId, idItem = riuLv1,cellNum = 2, num = 1, position = positionItem.hand, lv = 1},
        }
...
PackageHandlers.registerServerHandler("TakingMissionTutorial", function(player, packet)
    ...
    Trigger.CheckTriggers(this, "PALYER_CHECK_TUTORIAL_MISSION", {obj1 = player, model = Gol.Player[player.platformUserId]}) 
end)
